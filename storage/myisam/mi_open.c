    ...
    share->blocksize=MY_MIN(IO_SIZE,myisam_block_size);
    {
      ...
      for (i=0 ; i < keys ; i++)
      {
        ...
        if (keyinfo->flag & HA_SPATIAL)
          keyinfo->keysegs-= (sp_segs= 2*SPDIMS);
        else
          sp_segs= 0;
        for (j=0 ; j < keyinfo->keysegs + sp_segs; j++, pos++)
	{
              ...
              pos->type == HA_KEYTYPE_VARTEXT2)
	  {
	    ...
	    pos->charset= &my_charset_bin;
          if (!sp_segs || (j < keyinfo->keysegs))
            set_if_bigger(share->vreclength, pos->start + pos->length);
	}
        if (keyinfo->flag & HA_SPATIAL)
	{
#ifdef HAVE_SPATIAL
